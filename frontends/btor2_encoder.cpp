...
// when we encounter it again we shall replace the state's name
    ...
    void BTOR2Encoder::parse_without_constraint(const std::string &filename) {
        FILE *input_file = fopen(filename.c_str(), "r");
        ...
        while ((l_ = btor2parser_iter_next(&it_))) {
            ...
            if (l_->tag == BTOR2_TAG_state) {
                ...
                if (ts_.only_curr(constraint)) {
                    constraints_.push_back(constraint);
                } else {
                    throw PonoException("Constraints over next state variables or input are not supported");
                }
                terms_[l_->id] = constraint;
            } else if (l_->tag == BTOR2_TAG_init) {
                if (termargs_.size() != 2) {
    ...
    smt::Term BTOR2Encoder::prop() const {
        ...
        if (propvec_.size() > 1) {
            logger.log(defines::logWarning, 0, "More than one property, using first");
        }
        auto prop = propvec_.at(0);
        return prop;
...
//        }
    ...
    void BTOR2Encoder::decode_without_constraint(const std::string &path,
                                                 TransitionSystem &transitionSystem,
                                                 smt::Term &prop) {
        auto encoder = BTOR2Encoder(path, transitionSystem, true);
        prop = encoder.prop();
//        if (!encoder.constraints_.empty()) {
//            transitionSystem.add_init(transitionSystem.make_term(And, encoder.constraints_));
//        }
    }
